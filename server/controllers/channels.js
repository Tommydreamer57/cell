...
function create(req, res) {
    ...
    console.log({ organization_id, name, _private, created_by });
    if (!req.user.organizations.includes(+organization_id)) {
        res.status(403).send("user: " + user_id + " not a member of organization: " + organization_id);
    } else {
        req.db.create_channel({ organization_id, created_by, name, _private })
            .then(([channel]) => {
...
function join(req, res) {
    ...
    if (req.user.channels.includes(+channel_id)) {
        ...
        req.db.create_channel_membership({ channel_id, user_id })
            .then(() => {
                res.redirect(`/api/organization?channel_id=${channel_id}`);
            })
            .catch(err => {
                console.log(err);
                res.status(500).send(err);
